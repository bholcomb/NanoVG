...
int loadDemoData(NVGcontext* vg, DemoData* data)
{
	...
	data->fontIcons = nvgCreateFont(vg, "icons", "../data/fonts/entypo.ttf");
	if (data->fontIcons == -1) {
		printf("Could not add font icons.\n");
		return -1;
	}
	data->fontNormal = nvgCreateFont(vg, "sans", "../data/fonts/Roboto-Regular.ttf");
	if (data->fontNormal == -1) {
		printf("Could not add font italic.\n");
		return -1;
	}
	data->fontBold = nvgCreateFont(vg, "sans-bold", "../data/fonts/Roboto-Bold.ttf");
	if (data->fontBold == -1) {
		printf("Could not add font bold.\n");
		return -1;
	}
	data->fontEmoji = nvgCreateFont(vg, "emoji", "../data/fonts/NotoEmoji-Regular.ttf");
	if (data->fontEmoji == -1) {
